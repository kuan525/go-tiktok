...
func (c ConnectController) DouyinPublishAction(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinPublishActionHandler, &request.DouyinPublishActionReq{}, conf.Logger)
}

// DouyinPublishList 发布列表 登录用户的视频发布列表，直接列出用户所有投稿过的视频。
func (c ConnectController) DouyinPublishList(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinPublishListHandler, &request.DouyinPublishListReq{}, conf.Logger)
}
