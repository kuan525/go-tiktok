...
func (c ConnectController) DouyinMessageChat(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinMessageChatHandler, &request.DouyinMessageChatReq{})
}

// DouyinMessageAction 消息操作 登录用户对消息的相关操作，目前只支持消息发送
func (c ConnectController) DouyinMessageAction(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinMessageActionHandler, &request.DouyinMessageActionReq{})
}
