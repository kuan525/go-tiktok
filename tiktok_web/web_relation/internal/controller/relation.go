...
func (c ConnectController) DouyinRelationAction(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinRelationActionHandler, &request.DouyinRelationActionReq{}, conf.Logger)
}

// DouyinRelationFollowList 用户关注列表 登录用户关注的所有用户列表。
func (c ConnectController) DouyinRelationFollowList(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinRelationFollowListHandler, &request.DouyinRelationFollowListReq{}, conf.Logger)
}

// DouyinRelationFollowerList 用户粉丝列表 所有关注登录用户的粉丝列表。
func (c ConnectController) DouyinRelationFollowerList(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinRelationFollowerListHandler, &request.DouyinRelationFollowerListReq{}, conf.Logger)
}

// DouyinRelationFriendList 用户好友列表 所有关注登录用户的粉丝列表。
func (c ConnectController) DouyinRelationFriendList(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinRelationFriendListHandler, &request.DouyinRelationFriendListReq{}, conf.Logger)
}
