...
func (c ConnectController) DouyinFavoriteAction(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinFavoriteActionHandler, &request.DouyinFavoriteActionReq{})
}

// DouyinFavoriteList 喜欢列表 登录用户的所有点赞视频。
func (c ConnectController) DouyinFavoriteList(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinFavoriteListHandler, &request.DouyinFavoriteListReq{})
}
