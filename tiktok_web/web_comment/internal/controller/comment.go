...
func (c ConnectController) DouyinCommentAction(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinCommentActionHandler, &request.DouyinCommentActionReq{})
}

// DouyinCommentList 视频评论列表 查看视频的所有评论，按发布时间倒序。
func (c ConnectController) DouyinCommentList(ctx iris.Context) {
	middleware.AipWrapper(ctx, service.DouyinCommentListHandler, &request.DouyinCommentListReq{})
}
